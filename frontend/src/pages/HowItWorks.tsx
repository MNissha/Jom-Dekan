...
export default function HowItWorks() {
  return (
    <div className="overflow-hidden bg-[#FBFBFE] text-slate-800">
      <section id="journey" className="mx-auto max-w-6xl scroll-mt-24 px-[18px] py-16 sm:py-20">
        <Reveal className="text-center"><p className="text-xs font-extrabold uppercase tracking-[.2em] text-primary-600">How JomDekan works</p><h2 className="mt-3 text-3xl font-extrabold text-[#231C57] sm:text-4xl">From joining to getting study help</h2><p className="mx-auto mt-3 max-w-2xl text-slate-600">A practical student journey built around tools already available in JomDekan.</p></Reveal>
        <div className="relative mt-14 space-y-14 sm:space-y-20"><div className="absolute bottom-8 left-6 top-8 hidden w-px bg-gradient-to-b from-primary-300 via-amber-300 to-primary-200 lg:left-1/2 lg:block" aria-hidden="true" />{steps.map(({ number, icon: Icon, title, copy, visual }, index) => <Reveal key={number} className={`relative grid items-center gap-8 lg:grid-cols-2 lg:gap-16 ${index % 2 ? 'lg:[&>*:first-child]:order-2' : ''}`}><div className="relative"><span className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary-100 px-3 py-1 text-xs font-extrabold text-primary-700"><Icon className="h-4 w-4" />STEP {number}</span><h3 className="text-2xl font-extrabold text-[#231C57] sm:text-3xl">{title}</h3><p className="mt-3 max-w-xl leading-7 text-slate-600">{copy}</p><div className="mt-5 flex items-center gap-2 text-sm font-bold text-primary-700"><CheckCircle2 className="h-5 w-5 text-emerald-500" />Built into the current JomDekan experience</div></div><div className="transition duration-300 motion-safe:hover:-translate-y-1">{visual}</div></Reveal>)}</div>
      </section>

      <section className="border-y border-[#ECEBF7] bg-white px-[18px] py-20"><div className="mx-auto max-w-6xl"><Reveal><p className="text-xs font-extrabold uppercase tracking-[.2em] text-primary-600">One connected workspace</p><h2 className="mt-3 max-w-2xl text-3xl font-extrabold text-[#231C57]">Useful tools without losing your study flow</h2></Reveal><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4"><FeatureCard className="md:col-span-2 lg:row-span-2" icon={<UploadCloud />} title="Academic resources, properly organised" copy="Upload text or supported files, group multiple files in one resource, filter precisely and download what you need." large /><FeatureCard icon={<Bot />} title="AI study tools" copy="Summaries and grounded resource conversations for supported content." /><FeatureCard icon={<MessageSquare />} title="Student discussions" copy="Questions, replies, voting and solved threads." /><FeatureCard icon={<Heart />} title="Favourites" copy="Save resources, discussions and listings for later." /><FeatureCard icon={<ShieldCheck />} title="Safer participation" copy="Report inappropriate content for review without exposing admin tools." /></div></div></section>

      <section className="px-[18px] py-20"><Reveal className="relative mx-auto max-w-5xl overflow-hidden rounded-[28px] bg-[#231C57] px-6 py-12 text-center text-white shadow-xl sm:px-12"><div className="pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full bg-primary-500/30 blur-3xl" /><div className="relative"><GraduationCap className="mx-auto h-10 w-10 text-amber-300" /><h2 className="mt-4 text-3xl font-extrabold">Ready to make university life a little easier?</h2><p className="mx-auto mt-3 max-w-xl text-primary-100">Create your account and bring your resources, questions and study support together.</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><Link to="/register" className={`group inline-flex items-center justify-center gap-2 rounded-full bg-amber-400 px-6 py-3 font-bold text-[#231C57] transition hover:-translate-y-0.5 hover:bg-amber-300 ${focusRing}`}>Create your free account<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></Link><Link to="/" className={`inline-flex items-center justify-center rounded-full border border-white/40 px-6 py-3 font-semibold text-white transition hover:-translate-y-0.5 hover:border-white hover:bg-white/10 ${focusRing}`}>Back to home</Link></div></div></Reveal></section>
    </div>
  );
}
