...
import { ArrowLeft, Briefcase, Edit3, Eye, Plus, Search, Trash2, X } from "lucide-react";
  ...
  return <AdminPageShell embedded={embedded}>
    <div className="mx-auto max-w-6xl px-4 py-8 motion-safe:animate-[fadeIn_220ms_ease-out]">
      <header className="relative overflow-hidden rounded-[26px] bg-gradient-to-r from-[#332475] via-[#4338CA] to-[#6558DD] p-6 text-white shadow-lg">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#DDD8FF]">Marketplace administration</p><h1 className="mt-1 text-3xl font-bold">{pageLabel}</h1><p className="mt-1 text-sm text-[#D5D0F7]">Create, inspect and manage {category === "tutoring" ? "tutoring services" : "freelance opportunities"}.</p></div><button onClick={openCreate} className="inline-flex items-center gap-2 rounded-xl bg-[#F5C21A] px-4 py-2.5 text-sm font-bold text-[#231C57] shadow-md transition hover:-translate-y-0.5 hover:bg-amber-300"><Plus className="h-4 w-4" />Add new {category === "tutoring" ? "tutor" : "listing"}</button></div>
      </header>
      <div className="mt-5 grid gap-3 sm:grid-cols-3">{[["Total listings", categoryItems.length, "bg-indigo-50 text-indigo-600"], ["Active", active, "bg-emerald-50 text-emerald-600"], ["Closed", categoryItems.length - active, "bg-red-50 text-red-600"]].map(([label, value, color]) => <div key={label as string} className="flex items-center gap-3 rounded-2xl border border-[#E8E5F7] bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><span className={`rounded-xl p-2.5 ${color}`}><Briefcase className="h-5 w-5" /></span><div><p className="text-xs font-semibold text-slate-500">{label as string}</p><p className="text-2xl font-bold">{value as number}</p></div></div>)}</div>
...
function InspectModal({ item, close }: { item: Opportunity; close: () => void }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true"><div className="flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-[26px] bg-white shadow-2xl motion-safe:animate-[modalRise_220ms_ease-out]"><div className="bg-gradient-to-r from-[#332475] to-[#4B3CC4] p-5 text-white"><p className="text-xs font-bold uppercase tracking-widest text-[#D6D0FF]">Listing inspection</p><h2 className="mt-1 text-2xl font-bold">{item.title}</h2></div><div className="overflow-y-auto p-6"><div className="grid gap-3 sm:grid-cols-3"><Detail label="Owner" value={item.owner_name ?? "Unknown"} /><Detail label="Mode" value={item.mode} /><Detail label="Status" value={item.status.toUpperCase()} /></div><section className="mt-5 rounded-2xl border border-[#E8E5F7] bg-[#FAF9FF] p-5"><h3 className="text-xs font-bold uppercase tracking-wide text-[#4338CA]">Full listing information</h3><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-7 text-slate-700">{item.description}</p></section><p className="mt-4 text-xs text-slate-500">Created {new Date(item.created_at).toLocaleString()}</p></div><div className="border-t p-5"><button onClick={close} className="inline-flex items-center gap-2 rounded-xl border border-[#DDD9F1] px-5 py-2.5 text-sm font-bold text-slate-700 hover:bg-[#F8F7FD]"><ArrowLeft className="h-4 w-4" />Back</button></div></div></div>;
}

function Detail({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-[#E8E5F7] p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 break-words text-sm font-semibold text-slate-800">{value}</p></div>; }
